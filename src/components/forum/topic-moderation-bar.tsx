...
export function TopicModerationBar({
  topicId,
  pinned,
  locked,
}: {
  ...
  async function handleDelete() {
    if (!confirm("Ștergi acest topic și toate răspunsurile? Nu se poate anula.")) return;
    setBusy(true);
    try {
  ...
  return (
    <div className="flex items-center gap-2 panel rounded-md px-3 py-2 mb-4">
      ...
      <Button variant="ghost" size="sm" disabled={busy} onClick={() => patch({ pinned: !pinned })}>
        {pinned ? <PinOff size={14} /> : <Pin size={14} />} {pinned ? "Unpin" : "Pin"}
      </Button>
      <Button variant="ghost" size="sm" disabled={busy} onClick={() => patch({ locked: !locked })}>
        {locked ? <Unlock size={14} /> : <Lock size={14} />} {locked ? "Deblochează" : "Blochează"}
      </Button>
      <Button variant="ghost" size="sm" disabled={busy} onClick={handleDelete}>
        {busy ? <Loader2 className="animate-spin" size={14} /> : <Trash2 size={14} className="text-red-400" />}
        Șterge topic
      </Button>
    </div>
  );
