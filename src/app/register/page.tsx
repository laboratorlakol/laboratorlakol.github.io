  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ ...form, turnstileToken, tosAccepted: true }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.message ?? "A apărut o eroare.");
        // Reset Turnstile after any error so user doesn't need to refresh
  ...
  return (
    <AuthLayout title="Creează Cont" subtitle="Înregistrare gratuită.">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="font-mono text-xs uppercase tracking-wider text-ink-faint">Username</label>
          <Input className="mt-1.5" value={form.username} onChange={e => setForm({ ...form, username: e.target.value })}
            placeholder="numele_tau" autoComplete="username" required />
        </div>
        <div>
        ...
        <div>
          ...
          <Input className="mt-1.5" type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })}
            placeholder="Minimum 8 caractere" autoComplete="new-password" required />
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-xs text-ink-faint leading-relaxed">
            Am citit și accept{" "}
            <Link href="/tos" target="_blank" className="text-signal hover:underline">Termenii și Condițiile</Link>
            {" "}și{" "}
            <Link href="/privacy" target="_blank" className="text-signal hover:underline">Politica de Confidențialitate</Link>
            {" "}ale FADED Romania Roleplay.
          </span>
        </label>
        ...
        <Button
          ...
          variant="primary"
          className="w-full"
          disabled={loading || !tosAccepted || !turnstileToken}
        >
      ...
      <p className="mt-6 text-center text-sm text-ink-faint">
        Ai deja cont? <Link href="/login" className="text-signal hover:underline">Conectează-te</Link>
      </p>
    </AuthLayout>
  );
}
