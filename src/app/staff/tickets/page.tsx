...
export default function StaffTicketsPage() {
  ...
  async function load(status = "") {
    setLoading(true);
    try {
      const res = await fetch(`/api/staff/tickets${status ? `?status=${status}` : ""}`);
      if (!res.ok) {
        setForbidden(true);
        return;
      }
      ...
      <main className="min-h-screen bg-void px-6 pt-32 pb-24">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-1 text-sm text-ink-faint">
            Vezi doar categoriile pentru care rangul tău are acces, plus orice ticket în care ai fost adăugat.
          </p>
          ...
          {forbidden && (
            <div className="mt-8 flex items-start gap-3 panel border-glow rounded-md p-4">
              <ShieldAlert className="text-signal shrink-0 mt-0.5" size={18} />
              <p className="text-sm text-ink-muted">
                Rangul tău nu are acces la nicio categorie de tickete.
              </p>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="mt-6 panel rounded-md divide-y divide-line">
              {tickets.length === 0 && !forbidden && (
                <p className="px-4 py-8 text-center text-ink-faint">Niciun ticket.</p>
              )}
              {tickets.map((t) => (
                <Link
                  key={t.id}
                  href={`/suport/${t.id}`}
                  className="flex items-center justify-between px-4 py-3 hover:bg-panel-raised transition-colors"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-ink-faint mt-0.5">
                        {t.user.username} · {CATEGORY_LABEL[t.category]} · {t._count.messages} mesaje
                      </p>
                    </div>
                  </div>
                  <span className={`font-mono text-[10px] uppercase tracking-wider shrink-0 ml-4 ${STATUS_COLOR[t.status]}`}>
